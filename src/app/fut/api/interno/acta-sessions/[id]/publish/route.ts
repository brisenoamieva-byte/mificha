...
import { publicActaSession, publishActaSession } from "@/lib/match-acta-server";
...
/** Fuerza publicación (firmas OK o disputa resuelta por organizador). */
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const session = await publishActaSession(auth.admin, id);
