...
export function PlatformSeasonSection() {
  ...
  const loadPlatformSeasons = useCallback(async () => {
    setLoading(true);
    try {
      const payload = (await authedFetch("/fut/api/interno/platform-seasons")) as {
  ...
  async function handleCreatePlatformSeason(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await authedFetch("/fut/api/interno/platform-seasons", {
        method: "POST",
        body: JSON.stringify({
          name: name.trim(),
          region: region.trim(),
  ...
  async function assignToPublicAcademies(platformSeasonId: string) {
    setAssigning(true);
    try {
      const result = await authedFetch("/fut/api/interno/platform-seasons", {
        method: "POST",
        body: JSON.stringify({
