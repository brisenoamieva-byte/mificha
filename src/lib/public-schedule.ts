...
export async function fetchPublicUpcomingMatches(
  ...
  try {
    ...
    const matchesJson: unknown = await matchesResponse.json();
    const matches = Array.isArray(matchesJson)
      ? (matchesJson as Array<PublicScheduledMatch & { academy_id: string }>)
      : [];
    ...
    const academiesJson: unknown = await academiesResponse.json();
    const academies = Array.isArray(academiesJson)
      ? (academiesJson as Array<{
          id: string;
          name: string;
          slug: string;
